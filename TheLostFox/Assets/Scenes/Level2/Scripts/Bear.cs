...
public class Bear : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!isMeet)
            return;
        Move();
        animator.SetFloat("direction", rb.linearVelocity.x);
        animator.SetFloat("lookx", look);
        if (!Mathf.Approximately(rb.linearVelocity.x, 0))
            look = rb.linearVelocity.x;
        if(!isOnGround())
            isMakingTremor = true;
        ...
        if(starCountDown <= 0)
        {
            ...
            Instantiate(startPrefab, (Vector2)transform.position + Vector2.down * 2.3f, Quaternion.identity);
    ...
    void Death()
    {
        ...
        Destroy(gameObject);
        Instantiate(keyPrefab, (Vector2)transform.position + Vector2.down * 1.6f, Quaternion.identity);
